...
####
####
def _get_validation_segments(y, data_info: _DataInfo, n_repeats: int = 2):
    """
    Returns a list of validation segments [val_1, val_2, ...] each of length t_validate.
    Uses data_info.validation_start as the start of the validation block.
    """
    t = data_info.t_validate
    start = data_info.validation_start  # can be negative
    block = y[start:] if start < 0 else y[start:start + n_repeats * t]

    required = n_repeats * t
    if len(block) < required:
        raise ValueError(f"Validation block too short: {len(block)} samples, need {required}.")

    return [block[i * t:(i + 1) * t] for i in range(n_repeats)]


def _check_validate_replicate(
    input_path,
    output_path,
    delay: int,
    silent: bool,
    data_info: _DataInfo,
    n_repeats: int = 2,
    esr_replicate_threshold: float = 0.01,
    clip_warn_threshold: float = 0.999,
) -> _metadata.DataChecks:
    with _torch.no_grad():
        if not silent:
            print(f"V{data_info.major_version} checks (validate replicate)...")

        rate = data_info.rate
        y = _wav_to_tensor(output_path, rate=rate)

        # Too-short file for expected validation block?
        required = n_repeats * data_info.t_validate
        if len(y) < required:
            if not silent:
                print(f"Output too short: {len(y)} samples, need >= {required}.")
            return _metadata.DataChecks(version=data_info.major_version, passed=False)

        # Clipping warning
        y_abs_max = float(_torch.max(_torch.abs(y)).item())
        if y_abs_max >= clip_warn_threshold and not silent:
            print(f"WARNING: Output appears clipped (max abs = {y_abs_max:.6f}).")

        # Slice validation segments using data_info.validation_start
        try:
            vals = _get_validation_segments(y, data_info, n_repeats=n_repeats)
        except ValueError as e:
            if not silent:
                print(str(e))
            return _metadata.DataChecks(version=data_info.major_version, passed=False)

        # Replicate ESR between first two segments
        esr_replicate = _ESR(vals[0], vals[1]).item()
        if not silent:
            print(f"Replicate ESR is {esr_replicate:.8f}.")

        passed = True
        if esr_replicate > esr_replicate_threshold:
            # NOTE: pass both args if your message helper expects them
            if not silent:
                print(_esr_validation_replicate_msg(esr_replicate, esr_replicate_threshold))
            passed = False

            if not silent:
                _plt.figure()
                t_axis = _np.arange(len(vals[0])) / float(rate)
                _plt.plot(t_axis, vals[0], label="Validation 1")
                _plt.plot(t_axis, vals[1], label="Validation 2")
                _plt.xlabel("Time (sec)")
                _plt.legend()
                _plt.title(f"V{data_info.major_version} check: Validation replicate FAILURE")
                _plt.grid()
                _plt.show()

        return _metadata.DataChecks(version=data_info.major_version, passed=passed)
...
def _detect_input_version(input_path) -> _Tuple[_Version, bool]:
    ...
    def detect_weak(input_path) -> _Optional[_Version]:
        ...
        def detect_custom_v5(path) -> _Optional[_Version]:
            print("Beginning v5 checks...")
            x, info = _wav_to_np(path, info=True)
            rate = info.rate

            # Require 48k since the rest of the pipeline assumes STANDARD_SAMPLE_RATE for v5
            if rate != _V5_DATA_INFO.rate:
                return None

            # Look only at first 5 seconds for detection
            n5 = int(5 * rate)
            if x.shape[0] < n5:
                return None
            ...
            # Envelope for robust event detection
            block = max(1, rate // 100)  # 10ms blocks at 48k -> 480 samples
            m = (absx.shape[0] // block) * block
            if m <= 0:
                return None

            env = absx[:m].reshape(-1, block).max(axis=1)

            peak = float(env.max())
            if peak < 0.2:  # too quiet to be a blip-based file
                return None

            # Threshold relative to the strongest event
            thr = 0.7 * peak
            idx = _np.where(env >= thr)[0]
            if idx.size == 0:
                return None

            # Cluster blocks into events (events separated by >= 150ms count as distinct)
            gap = 15  # 150ms at 10ms/block => 15 blocks
            events = [idx[0]]
            for i in idx[1:]:
                if i - events[-1] >= gap:
                    events.append(i)

            # Convert event block index -> seconds
            t_events = _np.array(events) * (block / rate)

            # --- NEW: v5 should have blips very early (per _V5_DATA_INFO) ---

            # 1) Noise interval should be relatively quiet vs peak
            n0, n1 = _V5_DATA_INFO.noise_interval
            if n1 <= n0 or n1 > absx.shape[0]:
                return None

            noise_peak = float(absx[n0:n1].max())
            if noise_peak > 0.25 * peak:
                return None

            # 2) At least one event should fall in the configured blip_locations window(s)
            blip_ok = False
            for (s0, s1, s2) in _V5_DATA_INFO.blip_locations:
                t0 = s0 / rate
                t1 = s1 / rate
                t2 = s2 / rate
                if _np.any((t_events >= t0) & (t_events <= t2)):
                    blip_ok = True
                    break

            if blip_ok:
                print(f"Detected custom v5 input (events at ~{t_events[:6]} s)")
                return _Version(5, 0, 0)

            return None   
            

        v5 = detect_custom_v5(input_path)
        if v5 is not None:
            print("v5 is working!")
            return v5
...
_V5_DATA_INFO = _DataInfo(
    ...
    t_blips=int(0.6 * STANDARD_SAMPLE_RATE),       # 24000
    first_blips_start=0,

    # Validation: two identical 5-second chunks at the end
    t_validate=int(5.0 * STANDARD_SAMPLE_RATE),    # 240000
    ...
    # Your generator's clicks are at ~10%, 30%, 45% of the 0.5s blip region:
...
def _analyze_latency(
    ...
    elif input_version == _Version(5, 0, 0):
        calibrate, plot = _calibrate_latency_v5, _plot_latency_v5
        # (Choose whichever version in your file already uses blips.)
        #return calibrate_input_v4(input_path, output_path, **kwargs)
    ...
    if user_latency is not None:
        d = int(user_latency)
        print(f"Delay is specified as {d}. Skipping automatic calibration.")

        # --- Build a valid LatencyCalibration object without running calibrate() ---
        ...
        def _build_model(model_cls, overrides: dict):
            """
            Create a valid Pydantic v2 model instance by:
            - applying overrides
            - using defaults/default_factory where available
            - filling required fields with basic type defaults
            - recursively building nested BaseModel fields
            """
            data = dict(overrides)
            ...
            for name, field in model_cls.model_fields.items():
                ...
                # If there's a default_factory, call it
                ...
                # Fallback basic defaults by type
                data[name] = _default_for_type(ann)

            return model_cls.model_validate(data)

        # Construct LatencyCalibration with required overrides
        calib = _build_model(
            _metadata.LatencyCalibration,
            overrides={
                "algorithm_version": 0,   # must be int in your build
                "delays": [d],
                "safety_factor": 1,
                "recommended": d,
                "analyzed": d,
                # warnings will be auto-built as LatencyCalibrationWarnings model
            },
        )

        return _metadata.Latency(
            manual=d,
            calibration=calib,
        )
    ...
    if (not silent) and (calibration_output is not None) and (calibration_output.recommended is not None):
        plot(calibration_output.recommended, input_path, output_path)

    return _metadata.Latency(manual=None, calibration=calibration_output)
...
def _check_validate_replicate(
    ...
    *,
    ...
    """
    Generic check for formats whose output contains back-to-back identical validation
    chunks (replicates). Uses data_info.validation_start as the start of the validation
    ...
    with _torch.no_grad():
        ...
        # Basic sanity: file long enough for validation repeats
        required = n_repeats * data_info.t_validate
        if len(y) < required:
            if not silent:
                print(f"Output too short: {len(y)} samples, need >= {required}.")
        ...
        # Slice the validation block using validation_start
        start = data_info.validation_start
        val_block = y[start:] if start < 0 else y[start:]
        if len(val_block) < required:
            if not silent:
                print(
                    f"Validation block too short: {len(val_block)} samples, need >= {required}."
                )
            return _metadata.DataChecks(version=data_info.major_version, passed=False)

        t = data_info.t_validate
        vals = [val_block[i * t:(i + 1) * t] for i in range(n_repeats)]

        # Replicate ESR between first two validation chunks
        esr_replicate = _ESR(vals[0], vals[1]).item()
        if not silent:
            print(f"Replicate ESR is {esr_replicate:.8f}.")
        ...
        if esr_replicate > esr_replicate_threshold:
            if not silent:
                # adjust call signature if needed
                try:
                    print(_esr_validation_replicate_msg(esr_replicate, esr_replicate_threshold))
                except TypeError:
                ...
                _plt.figure()
                tt = _np.arange(len(vals[0])) / float(rate)
                _plt.plot(tt, vals[0], label="Validation 1")
                _plt.plot(tt, vals[1], label="Validation 2")
                _plt.xlabel("Time (sec)")
                _plt.legend()
                _plt.title(f"V{data_info.major_version} check: Validation replicate FAILURE")
                _plt.grid()
...
#class _DataInfo:
#    rate: int
#    t_validate: int  # samples

# Example: 48kHz, 2 seconds validate segment
#_V5_DATA_INFO = _DataInfo(
#    rate=48_000,
#    t_validate=int(2.0 * 48_000),
#)
###
###

#def _check_v5(input_path, output_path, delay: int, silent: bool) -> _metadata.DataChecks:
def _check_v5(input_path, output_path, delay: int, silent: bool) -> _metadata.DataChecks:
    # delay currently unused for this synthetic format
    return _check_validate_replicate(
        output_path=output_path,
        silent=silent,
        data_info=_V5_DATA_INFO,
        n_repeats=2,
        esr_replicate_threshold=0.01,
    )
...
def _check_v5_OLD(input_path, output_path, silent: bool, *args, **kwargs) -> _metadata.DataChecks:
    """
    V5 (synthetic) checks.
    Assumes output ends with two back-to-back identical validation chunks:
        ... | val_1 | val_2
    """
    with _torch.no_grad():
        print("V5 checks (synthetic)...")
        ...
        # Too-short file?
        required = 2 * _V5_DATA_INFO.t_validate
        if len(y) < required:
            print(
                f"Output too short for V5 validation: {len(y)} samples, need >= {required}."
            )
            return _metadata.DataChecks(version=5, passed=False)

        # Clipping check (helps with synthetic signals + loud amps)
        y_abs_max = float(_torch.max(_torch.abs(y)).item())
        if y_abs_max >= 0.999:
            print(f"WARNING: Output appears clipped (max abs = {y_abs_max:.6f}).")
            # Not always fatal, but usually a red flag:
            # passed = False

        # --- Replicate validation ESR ---
        t_validate = _V5_DATA_INFO.t_validate
        y_val_1 = y[-2 * t_validate : -t_validate]
        y_val_2 = y[-t_validate:]
...
def _check_data(
    ...
    # Issue 442: Deprecate inputs (make this explicit)
    DEPRECATED_MAJORS = {1, 2, 4}  # keep v3 + v5 allowed
    if input_version.major in DEPRECATED_MAJORS:
        print(
        ...
        out = _metadata.DataChecks(version=out.version, passed=False)
...
###########
def _get_major_version(data_info) -> int:
    # Older datasets (v1–v4) used a dedicated attribute
    if hasattr(data_info, "major_version"):
        return int(data_info.major_version)

    # Newer datasets (v5+) often store a full version (e.g., "5.0.0" or (5,0,0))
    v = getattr(data_info, "version", None)
    if v is None:
        v = getattr(data_info, "input_version", None)

    if v is None:
        raise AttributeError("Data info has no major_version, version, or input_version.")

    if hasattr(v, "major"):  # semantic version object
        return int(v.major)
    ...
    raise TypeError(f"Unrecognized version format on data_info: {type(v)}")
###########
...
def _get_data_config(
    ...
    def get_split_kwargs(data_info: _DataInfo):
        major = _get_major_version(data_info)
        if major == 1:
